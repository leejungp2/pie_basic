...
print(all([1,2,3])) # and - 모두 참이면 true, 하나라도 거짓이면 false
print(any([1,2,0])) # or
...
print(min([1,2,3]))
print(min('python study')) # 최소값 공백이 나옴

# 중요! map: 반복 가능한 객체 요소를 지정한 함수 실행 후 추출
def conv_abs(x):
    return abs(x)
print(list(map(conv_abs,[1,-3,2,9,-5,6])))

print(list(map(lambda x:abs(x), [1,-3,2,9,-5,6])))
